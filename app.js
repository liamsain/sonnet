// Utility function to get a random sonnet
...
function getRandomSonnet() {
    const lines = sonnets.split('\n');
    
    // Parse sonnets (separated by blank lines)
    const sonnetArray = [];
    let currentSonnet = [];
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line === '') {
            if (currentSonnet.length > 0) {
                sonnetArray.push(currentSonnet);
                currentSonnet = [];
            }
        } else {
            currentSonnet.push(line);
        }
    }
    if (currentSonnet.length > 0) {
        sonnetArray.push(currentSonnet);
    ...
    // Filter out sonnets with less than 14 lines (some might be incomplete)
    const validSonnets = sonnetArray.filter(s => s.length >= 14);
    ...
    // Get random sonnet and take first 14 lines
    const randomIndex = Math.floor(Math.random() * validSonnets.length);
    return validSonnets[randomIndex].slice(0, 14);
}
